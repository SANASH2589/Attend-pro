...
import smsApi, { SmsLog } from '../../api/sms';
...
export default function SmsLogs() {
  ...
  const [dateFrom, setDateFrom] = useState<string>(getPastDate(7));
  ...
  const fetchStats = useCallback(async () => {
    try {
      const data = await smsApi.getSmsStats();
      setSentToday(data.sentToday || 0);
      setFailedCount(data.failedToday || 0);
      setTotalWeek(data.totalWeek || 0);
    } catch (err: any) {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      let filteredLogs: SmsLog[] = result.logs || [];
  ...
  const handleRetryRow = async (log: SmsLog) => {
  ...
  const columns: TableColumn<SmsLog>[] = [
    {
      label: 'Sent at',
      key: 'sent_at',
      render: (row) => (
