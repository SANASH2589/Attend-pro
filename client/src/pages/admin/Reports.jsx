      ...
      {!loading && report && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <StatCard title="SMS Sent" value={String(report.overall.total_sms_sent)} icon={MessageSquare} description="Notifications dispatched" />
            <StatCard
              title="SMS Failed"
              value={String(report.overall.total_sms_failed)}
              icon={AlertTriangle}
              description="Delivery failures"
              className={report.overall.total_sms_failed > 0 ? '' : ''}
            />
          </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-sm">
                      {report.low_attendance_students.map((s, i) => (
