...
export default function AttendanceMonitoring() {
  ...
  const [dateFrom, setDateFrom] = useState<string>(getPastDateString(7));
  const [dateTo, setDateTo] = useState<string>(getPastDateString(0));
  const [sessionSegment, setSessionSegment] = useState<string>('all');
  ...
  // SMS logs in drawer
  const [drawerSmsLogs, setDrawerSmsLogs] = useState<SmsLog[]>([]);
  const [smsRetrying, setSmsRetrying] = useState<boolean>(false);

  const fetchStats = async () => {
    try {
      const todayStr = new Date().toISOString().split('T')[0];
      
      // Fetch today's session logs
  ...
  const fetchSessionsList = async () => {
    ...
    try {
      ...
      let sessionsData: AttendanceSession[] = result.sessions || [];
  ...
  const openDetailDrawer = async (sessionId: string) => {
    setDrawerOpen(true);
  ...
  const handleUnlockSession = async (sessionId: string) => {
    ...
    try {
      ...
      showToast('Session unlocked. An audit log entry has been created.', 'success');
      setConfirmingAction(null);
      fetchSessionsList();
      fetchStats();
    } catch (err: any) {
