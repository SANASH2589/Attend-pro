...
interface ExtendedClass extends ClassType {
  staff_count?: number;
}
...
export default function Classes() {
  ...
  const [deleteConflictError, setDeleteConflictError] = useState<string>('');

  // Modals state
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingClass, setEditingClass] = useState<ExtendedClass | null>(null);
  ...
  const fetchClasses = async () => {
    ...
    try {
      ...
      setClasses(data as ExtendedClass[]);
    } catch (err: any) {
  ...
  const formatWindow = (start?: string | null, lock?: string | null) => {
  ...
  const columns: TableColumn<ExtendedClass>[] = [
    {
    ...
    },
    {
      ...
      render: (row) => {
        const types = {
          morning: { label: 'Morning Only', variant: 'warning' as const },
          evening: { label: 'Evening Only', variant: 'neutral' as const },
          both: { label: 'Double Batch', variant: 'success' as const }
        };
        const config = types[row.batch_type] || { label: row.batch_type, variant: 'neutral' as const };
